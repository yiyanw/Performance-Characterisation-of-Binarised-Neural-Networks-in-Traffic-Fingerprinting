...
import numpy as np
import larq as lq
from IoT.utility import LoadDataIot, LoadDataIotNumeric, LoadDataIotNumeric_v2, LoadDataIotNumeric_v3, LoadDataIotNumeric_v4
...
def pre_process(use_thermo_encoding):
    ...
    if use_thermo_encoding == 'standard' or use_thermo_encoding == 'customized':
        ...
        X_train = np.reshape(X_train, (len(X_train), -1, channel_num, 1))
        X_train = np.transpose(X_train, [0, 2, 1, 3])
        X_train = np.reshape(X_train, [len(X_train), 5, -1, channel_num])

        X_valid = te.transform(X_valid)
        X_valid = np.reshape(X_valid, (len(X_valid),-1,channel_num, 1))
        X_valid = np.transpose(X_valid, [0, 2, 1, 3])
        X_valid = np.reshape(X_valid, [len(X_valid), 5, -1, channel_num])

        X_test = te.transform(X_test)
        X_test = np.reshape(X_test, (len(X_test),-1,channel_num, 1))
        X_test = np.transpose(X_test, [0, 2, 1, 3])
        X_test = np.reshape(X_test, [len(X_test), 5, -1, channel_num])
    # thermo encoding end

    else:
        X_train = X_train.reshape(X_train.shape[0], 5, -1, 1)
        X_valid = X_valid.reshape(X_valid.shape[0], 5, -1, 1)
